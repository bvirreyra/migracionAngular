...
export class SeguimientoExternoComponent implements OnInit {
  ...
  public m_codigo_carpeta: any;
  public m_nroPlanilla: any;
 ...
 /*******************************************************************************/
  ...
  paneles(string, dts?) {
    ...
    if (string == "ADJUNTOS") {
      ...
      $("#modalEliminaSupervision").modal("hide");
      $("#pnl_gridview_equipotecnico").hide();
      this.m_nroPlanilla = dts.nro_supervision;
      setTimeout(() => {
        this.pnl_adjuntos = true;
        console.log("dtsPla", dts, this.m_nroPlanilla);
      }, 100);
    }
  }
    ...
    this._seguimiento.listaProyectos().subscribe(
      (result: any) => {
        ...
        if (Array.isArray(result) && result.length > 0) {
          ...
          this.dts_seguimiento = dts.filter(
            (item) => item.sector == "EDUCACIÓN"
          );

          console.log(this.dts_seguimiento);
          ...
          setTimeout(() => {
            ...
            this._fun.selectTable(table, [1, 2, 5, 9]);
            this._fun.inputTable(table, [3, 4]);
          }, 500);
        } else {
  ...
  pre_listaSupervision(dts, nombre_proyecto) {
    this.m_nombreproyecto = nombre_proyecto;
    this.m_codigo_carpeta = dts.codigo_carpeta;
    this.paneles("VER_GRIDVIEW_SUPERVISION");
