...
@Component({
  selector: "app-bandeja-financiero",
  templateUrl: "./bandeja-financiero.component.html",
  styleUrls: ["./bandeja-financiero.component.css"],
  providers: [
...
export class BandejaFinancieroComponent implements OnInit, OnChanges {
  ...
  mostrar: boolean;
  monto_convenio: any;
  monto_contraparte: any;
  monto_upre: any;
  dts_listaConvenio: any;
  inputAccion: any;
  inputNombre: any;
  inputCodigoProy: any;
  inputTipoImagen: any;
  inputRepositorio: any;
  pnlInfraestructura: any;
  pnlSupervision: any;
  pnlFiscalizacion: any;
  dts_planillas2Inicial: any;
  pnlEstructuraFinanciamiento: any;
  ...
  ngOnChanges() {
    this.obtieneDatosProyecto(this.inputDtsBandeja["_id_proyecto"]).then(
      (dts) => {
        this.pnlPrincipalFinanciera = true;
        this.inputDts = dts;
        ...
        this.obtenerConexion()
          ...
          .then((dts) => {
            ...
            this.mostrar = true;
            var totales = alasql(
              `SELECT  SUM(cast(monto_upre as decimal)) as monto_upre,
              SUM(cast(monto_contraparte_municipal as decimal)) as monto_municipal,
              SUM(cast(monto_contraparte_gobernacion as decimal)) as monto_gobernacion,
              SUM(cast(monto_contraparte_beneficiario as decimal)) as monto_beneficiario FROM ?  `,
              [dts]
            );
            this.monto_convenio = (
              this._fun.valorNumericoDecimal(totales[0].monto_upre) +
              this._fun.valorNumericoDecimal(totales[0].monto_municipal) +
              this._fun.valorNumericoDecimal(totales[0].monto_gobernacion) +
              this._fun.valorNumericoDecimal(totales[0].monto_beneficiario)
            ).toFixed(2);
            //this.monto_total = this.monto_total.toFixed(2);

            this.monto_contraparte = (
              this._fun.valorNumericoDecimal(totales[0].monto_municipal) +
              this._fun.valorNumericoDecimal(totales[0].monto_gobernacion) +
              this._fun.valorNumericoDecimal(totales[0].monto_beneficiario)
            ).toFixed(2);
            this.monto_upre = this._fun.valorNumericoDecimal(
              totales[0].monto_upre
            );
            console.log(
              this.monto_convenio,
              this.monto_contraparte,
            ...
            this.pnlEstructuraFinanciamiento = true;
            return this.lista_convenio(this.inputDts.v_id_proyecto);
          })
          .then((dts) => {
            ...
            if (
              ...
              this.inputTipoImagen = "";
              this.inputRepositorio = this.dts_listaConvenio[0].repositorio;
            }
          });
  ...
  detallar(valor: string, planilla?: number) {
    ...
    if (valor == "desembolsos") {
      if (this.pnl_desembolsos) {
        ...
        this.pnlEstructuraFinanciamiento = false;
      }
    }
    if (valor == "detalles") {
      ...
      this.cargaDesembolsos()
        ...
        .then((dts: any) => {
          if (dts.length > 0) {
            ...
            this.dts_ef_totales = alasql(
              `SELECT  estructura_financiamiento,SUM(cast(liquido_apagar as decimal)) total_pagado FROM ? group by estructura_financiamiento `,
              [dts]
            );

            //combinando datos de estructura financiamiento con total desembolsado estructura financiamiento
            this.dts_estructuraFinanciamiento = alasql(
              `select a.estructura_financiamiento,a.id_detalle,a.id_estado,a.id_proyecto,a.monto_contraparte_beneficiario,a.monto_contraparte_gobernacion,
            a.monto_contraparte_municipal,a.monto_upre,b.total_pagado
            FROM ? as a  left join ? as b on a.estructura_financiamiento=b.estructura_financiamiento`,
              [this.dts_estructuraFinanciamiento, this.dts_ef_totales]
            );
            ...
            console.log("TOTALES SEGUN EF===>", this.dts_ef_totales);
          }
  ...
  planillas2() {
    ...
    this.dts_planillas2Inicial = this.dts_planillas2;
    ...
    setTimeout(() => {
      ...
      console.log("anticipo===>", this.dts_anticipos);
      if (this.dts_anticipos.length > 0) {
        for (var i = 0; i < this.dts_anticipos.length; i++) {
          this.total_monto_planilla =
            Number(this._fun.valorNumericoDecimal(this.total_monto_planilla)) +
            Number(
              this._fun.valorNumericoDecimal(this.dts_anticipos[i].anticipo)
            );
  ...
  listarProyectosEducacion() {
    ...
    this._seguimiento.listaProyectosConsolidados().subscribe(
      (result: any) => {
        console.log("proyectos educacion", result);
        if (Array.isArray(result) && result.length > 0) {
      ...
      swal2({
        ...
        if (result.value) {
          const laPlanilla = {
            ...
            liquidoPagar: 0,
            preventivo: null,
            fechaPago: idPlanilla.fecha_pago,
            estadoPlanilla: 0,
  ...
  switchDatosdelProyecto() {
    console.log("entra al switch");
    this.mostrar = !this.mostrar;
  }
  switchPaneles(dato) {
    console.log("respuesta hijo===>", dato);
    if (dato == "INFRAESTRUCTURA") {
      window.scrollTo(0, 0);
      this.pnlInfraestructura = true;
      this.pnlPrincipalFinanciera = false;
      this.dts_planillas2 = alasql(
        `select * from ? where estructura_financiamiento=?`,
        [this.dts_planillas2Inicial, dato]
      );

      this.sumatotales_estructurafinanciera();
    }
    if (dato == "DASHBOARD_PRINCIPAL") {
      window.scrollTo(0, 0);
      this.pnlInfraestructura = false;
      this.pnlPrincipalFinanciera = true;
      this.dts_planillas2 = this.dts_planillas2Inicial;
      this.pnlEstructuraFinanciamiento = true;
      this.pnl_desembolsos = false;
      this.pnl_detalles = false;
      this.sumatotales_estructurafinanciera();
    }
    if (dato != "DASHBOARD_PRINCIPAL" && dato != "INFRAESTRUCTURA") {
      window.scrollTo(0, 0);
      this.dts_planillas2 = alasql(
        `select * from ? where estructura_financiamiento=?`,
        [this.dts_planillas2Inicial, dato]
      ...
      this.pnlInfraestructura = false;
      this.pnlPrincipalFinanciera = false;
      this.sumatotales_estructurafinanciera_componente();
    }
  }
  sumatotales_estructurafinanciera() {
    this.total_monto_planilla = 0;
    this.total_descuentos = 0;
    this.total_desc139 = 0;
    this.total_desc140 = 0;
    this.total_desc141 = 0;
    this.total_desc142 = 0;
    this.total_desc143 = 0;
    this.total_liquido_apagar = 0;
    setTimeout(() => {
      this.total_monto_planilla = this.dts_planillas2.reduce((acc, obj) => {
        console.log("acc", this._fun.valorNumericoDecimal(acc));
        console.log("sig", this._fun.valorNumericoDecimal(obj.monto_planilla));
  ...
  sumatotales_estructurafinanciera_componente() {
    this.total_monto_planilla = 0;
    this.total_descuentos = 0;
    this.total_desc139 = 0;
    this.total_desc140 = 0;
  ...
  lista_convenio(id_proy) {
    return new Promise((resolve, reject) => {
      ...
      this._seguimiento.listaConvenioxIdSgp(id_proy).subscribe(
        (result: any) => {
          var lista_convenio;
          if (Array.isArray(result) && result.length > 0) {
