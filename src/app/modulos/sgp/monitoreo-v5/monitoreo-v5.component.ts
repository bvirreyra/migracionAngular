...
@Component({
  selector: "app-monitoreo-v5",
  templateUrl: "./monitoreo-v5.component.html",
  styleUrls: ["./monitoreo-v5.component.css"],
})
export class MonitoreoV5Component implements OnInit {
  ...
  totalesDepCIF: { costo: number; cantidad: number } = {
    costo: 0,
    cantidad: 0,
  };
  totalesDepCIFE: { costo: number; cantidad: number } = {
    costo: 0,
    cantidad: 0,
  };

  compromisosPresi: any[] = [];
  ...
  recibeMensaje(data: any, tipo: string) {
    console.log("recibiendo mensaje de chart", data, tipo, this.seriesBarDep);
    if (tipo == "bardep") {
      this.cargarPresidencia(this.seriesBarDep[data.s].name);

      setTimeout(() => {
        const obj = document.getElementById("grillaDetalle");
  ...
  armarSeries(dts: any[]) {
    ...
    const porDepFin = alasql(
      "select departamento,tipo_financiamiento, count(*) cantidad ,sum(cast(costo_proyecto as double)) monto from ? group by departamento,tipo_financiamiento order by departamento, tipo_financiamiento",
      [dts]
    );
    let modDep = [];
    let modArea = [];
    ...
    console.log("cris", porDepFin);
    this.seriesDepCIF = porDepFin.filter(
      (f) => f.tipo_financiamiento === "CIF"
    );
    this.seriesDepCIFE = porDepFin.filter(
      (f) => f.tipo_financiamiento === "CIFE"
    );
    this.periodosBarDep = [
      "Compromisos entre " +
    ...
    this.totalesDepCIF.costo = this.seriesDepCIF.reduce(
      (ac, el) => ac + Number(el.monto),
      0
    );
    this.totalesDepCIF.cantidad = this.seriesDepCIF.reduce(
      (ac, el) => ac + Number(el.cantidad),
      0
    );
    this.totalesDepCIFE.costo = this.seriesDepCIFE.reduce(
      (ac, el) => ac + Number(el.monto),
      0
    );
    this.totalesDepCIFE.cantidad = this.seriesDepCIFE.reduce(
      (ac, el) => ac + Number(el.cantidad),
      0
    );
  ...
  prepararTabla() {
    ...
    setTimeout(() => {
      ...
      if (!$.fn.dataTable.isDataTable(".dt-detalle")) {
        ...
        this._fun.inputTable(table, [1, 3, 4, 5, 6, 8]);
        this._fun.selectTable(table, [0, 2, 7, 9]);
      }
  ...
  cargarComparativa(compromiso: any) {
    console.log("cargando compromiso etapas", compromiso);
    // this.dtsDetallado = null;
    compromiso.id_compromiso = compromiso.id_compromiso;
    compromiso.id_flujo = compromiso.fid_flujo;
    this.elCompromiso = compromiso;
    console.log(this.elCompromiso, compromiso);
  ...
  cargarPresidencia(opcion: string) {
    this._sgp.compromisosPresidencia({ opcion: "T" }).subscribe(
      (result: any) => {
        
        console.log(result);
        this.tituloGrillaDetalle = "Compromisos Presidenciales: " + opcion;
        this.dtsDetallado = result.filter(
          (f) => f.departamento.toUpperCase() === opcion
        );
        console.log("filtrado", this.dtsDetallado);
