...
@Component({
  selector: "app-cabecera-sgp-norelacionados",
  templateUrl: "./cabecera-sgp-norelacionados.component.html",
  styleUrls: ["./cabecera-sgp-norelacionados.component.css"],
  providers: [
...
export class CabeceraSgpNorelacionadosComponent implements OnInit {
  ...
  paneles(string) {
    if (string == "VER_LISTAPROYECTOS") {
      this.m_titulo = "Proyectos Financiera Sin Relacionar con Sgp";
  ...
  listaproyectosparavalidacion() {
    ...
    this._seguimiento.listaProyectosParaConsolidarV2().subscribe(
      (result: any) => {
        if (Array.isArray(result) && result.length > 0) {
          this.dts_listaproyectos = this._fun.RemplazaNullArray(result);
          console.log("dts", this.dts_listaproyectos);
          ...
          setTimeout(() => {
            ...
            this._fun.selectTable(table, [1, 2, 3, 4, 14]);
            this._fun.inputTable(table, [5, 6, 7, 8, 9, 10]);
            this.cargando = false;
          }, 5);
        } else {
