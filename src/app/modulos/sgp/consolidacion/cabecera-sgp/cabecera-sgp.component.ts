...
@Component({
  selector: "app-cabecera-sgp",
  templateUrl: "./cabecera-sgp.component.html",
  styleUrls: ["./cabecera-sgp.component.css"],
  providers: [
...
export class CabeceraSgpComponent implements OnInit {
  ...
  paneles(string) {
    if (string == "VER_LISTAPROYECTOS") {
      this.m_titulo = "Proyectos Financiera Relacionados con el Sgp";
      this.listaproyectosparavalidacion();
    }
  }
  listaproyectosparavalidacion() {
    this.cargando = true;
    this._seguimiento.listaProyectosParaConsolidarV1().subscribe(
      (result: any) => {
