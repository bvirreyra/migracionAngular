...
@Component({
  selector: "app-inventariofisicovalorado",
  templateUrl: "./inventariofisicovalorado.component.html",
  styleUrls: ["./inventariofisicovalorado.component.css"],
  providers: [
...
export class InventariofisicovaloradoComponent implements OnInit {
  ...
  BuscarFecha() {
    //console.log(this.m_fechafin);
    this._almacenes.getListaInventarioFisicoValorado(this.m_fechafin).subscribe(
      (result: any) => {
        if (result.length > 0) {
  ...
  rpt_inventariofisicovalorado(datos: any) {
    console.log("cantidad", datos[0].cantidad);
    let nro_registros = datos.length;
    ...
    var interlineado = 20;
    var margenleft = 10;
    doc.text(margenleft + 55, interlineado, "INVENTARIO FÍSICO VALORADO");
    interlineado = interlineado + 5;
    doc.text(margenleft + 80, interlineado, "AL " + this.m_fechafin);
    interlineado = interlineado + 5;
    let cabecera = [
      { title: "CODIGO", dataKey: "cod_articulo" },
      { title: "DESCRIPCION DE MATERIAL", dataKey: "descripcion" },
      ...
      { title: "COSTO UNITARIO", dataKey: "costo_unitario" },
    ...
    dts.forEach((registro) => {
      let row = {
        ...
        costo_unitario: registro.costo_unitario.toLocaleString("de-DE", {
          style: "decimal",
          decimal: "2",
          minimumFractionDigits: 2,
        }),
        //costo_unitario: registro.costo_unitario.toFixed(2).toLocaleString(),
        ...
        //saldo_bs: registro.saldo_bs.toLocaleString('de-DE', { style: 'decimal', decimal: '2',maximumSignificantDigits: 2 }),
    ...
    let row2 = {
      ...
      //valor_salida: total_salidavalor.toLocaleString('de-DE', { style: 'decimal', decimal: '3' }),
      costo_unitario: "",
    ...
    doc.autoTable(cabecera, detalle, {
      //Propiedades
      margin: { horizontal: 10, top: interlineado },
      tableWidth: "wrap",
      ...
      columnStyles: {
        ...
        descripcion: { columnWidth: 50, halign: "right" },
    ...
    console.log(this.dtsFechaSrv, this.dtsHoraSrv, this.s_user);
    ...
    doc.save("rptInventarioFisicoValorado.pdf");
  }
}
